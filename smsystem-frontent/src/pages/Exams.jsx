...
import { examsApi } from '../api/school'
...
export default function Exams() {
  ...
  async function save(e) {
    ...
    try {
      if (editing) {
        await examsApi.update(editing.examId, p)
      } else {
        await examsApi.create(p)
      }
    ...
    <>
      <div className="page-head">
        <div>
          ...
          <h1>Kỳ thi</h1>
          <p>Lập lịch, quản lý trạng thái và cấu hình bài thi.</p>
        </div>
            ...
            <tbody>
              {loading ? (
                ...
                items.map(x => (
                  <tr key={x.examId}>
                    <td>
                      <strong>{x.examName}</strong>
                      <small>{x.examType || '—'}</small>
                    </td>
      ...
      <Modal
        ...
        title={editing ? 'Chỉnh sửa kỳ thi' : 'Tạo kỳ thi'}
        wide
      >
        <form
          className="form-grid"
